...
type User struct {
	...
	OAuthConnectorID uint    `gorm:"column:oauth_connector_id;type:int;default:0;comment:'oauth连接器ID'" json:"oauthConnectorId"`
	OAuthProvider    string  `gorm:"column:oauth_provider;type:varchar(50);comment:'oauth提供方'" json:"oauthProvider"`
	UidNumber        uint    `gorm:"type:int;default:0;comment:'POSIX uidNumber'" json:"uidNumber"`          // POSIX uid
	GidNumber        uint    `gorm:"type:int;default:0;comment:'POSIX gidNumber'" json:"gidNumber"`          // POSIX gid
	HomeDirectory    string  `gorm:"type:varchar(255);comment:'home目录'" json:"homeDirectory"`                // POSIX home
	LoginShell       string  `gorm:"type:varchar(64);comment:'登录shell'" json:"loginShell"`                   // POSIX shell
