...
func InitData() {
	...
	apis := []model.Api{
		...
		{
			Method:   "POST",
			Path:     "/user/revokeOriginalKeypair",
			Category: "user",
			Remark:   "撤销旧SSH证书/密钥（占位）",
			Creator:  "系统",
		},
		{
			Method:   "POST",
	...
	for i, api := range apis {
		...
		if errors.Is(err, gorm.ErrRecordNotFound) {
			...
			basePaths := []string{
				...
				"/user/info",
				"/user/issueSSHPubKey",
				"/user/revokeOriginalKeypair",
				"/user/changePwd",
				"/user/password/code",
				"/oauth/connectors/public",
				"/oauth/start",
				"/oauth/callback/:provider",
				"/oauth/start/password",
				"/menu/access/tree",
				"/log/operation/list",
			}
